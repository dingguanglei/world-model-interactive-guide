...
### 1.4 单领域主站与 Legacy 原则 (Single-Domain + Legacy)
> **"主站只维护具身智能；世界模型历史内容统一进入 legacy，只读归档。"**

- 主站唯一领域：**具身智能**（`world_model_interactive_guide/index.html`）
- 历史世界模型内容：`world_model_interactive_guide/legacy/`（归档，不纳入主站日更）
- 主站更新日志：`world_model_interactive_guide/09_update_log.html`
- 后续所有常规更新任务均按具身智能主站章节执行。
- **强制忽略 `legacy/`**：搜索、批量替换、自动化写入均不触达 `legacy/`。
- 主站关键章节（必须保留）：**行业全景、公司调研、产品调研、落地路线、技术架构方案、端侧芯片调研、仿真环境、数据工程、论文追踪、评测基准、社区讨论**。
- 主站调研优先级：**数据工程（采集/合成）、仿真环境、技术架构与端侧芯片、落地路线、公司与产品调研**（核心对应 `02_product.html`、`03_architecture.html`、`11_edge_chip.html`、`04_data.html`、`05_roadmap.html`、`06_companies.html`）。
...
- 必须给出“该评测对具身智能系统部署的真实价值”与“最容易踩的坑/刷分风险”，并建议如何把它用作工程回归测试（regression）而不是营销 KPI。
...
| 平台 | 监控内容 |
|------|---------|
| **X/Twitter** | #EmbodiedAI, #RobotLearning, @Figure_robot, @NVIDIAAIDev, @GoogleDeepMind |
...
> **核心原则**: 不仅仅是论文需要更新。任何与具身智能主站相关的内容有变化时，都必须及时更新，并记录在主站更新日志中。

### 6.1 更新范围 (Update Scope)
每次调研更新按具身智能主站章节顺序扫描；只在“确有高价值增量信息”时更新对应页面。
...
> 说明：`05_roadmap.html` 必须显式覆盖“落地路线 + 仿真环境（平台选型 / Sim2Real）”两部分内容，避免缺失任一关键章节维度；`11_edge_chip.html` 负责端侧芯片路线与部署选型深度调研。

#### 6.1.2 Legacy 约束（强制）
- `legacy/` 目录仅用于历史归档与回溯，不参与后续常规更新。
- 所有常规更新任务一律忽略 `legacy/`（不扫描、不改写、不批量替换）。

#### 6.1.3 质量优先（强制）
- 若某页面当天**没有高价值、可追溯**的新信息：**允许不更新该页面**（宁缺毋滥）。
- 允许“只更新部分页面”，但必须在 `09_update_log.html` 记录本次覆盖范围。
...
```
1. 按具身智能主站执行多维度搜索:
   - site:arxiv.org embodied intelligence robotics policy learning [年份]
...
**必须维护主站更新日志页面** (`09_update_log.html`)，记录具身智能主站内容变更。

#### 6.3.1 Legacy 排除说明（强制）
- `legacy/` 目录不参与后续常规更新，因此主站更新日志不记录 `legacy/` 变更项。
...
### 6.1.1 更新范围（必须关注）
- **机器人产品发布**：人形/移动操作/特种机器人产品发布、版本升级、部署进展。
- **系统能力演进**：感知、规划、控制、恢复、安全约束等关键能力变化。
- **部署与商业化信号**：试点场景、规模化部署、合作生态、服务模式变化。

### 6.1.2 每个产品条目必备信息（建议结构）
- **产品名称 + 官网/官方演示链接（必须可点击）**
- **核心能力清单**：任务成功率、恢复能力、控制延迟、安全机制、可持续运行稳定性
- **技术来源可追溯**：对应论文/技术报告/官方博客/GitHub
- **对比表**：与同类系统在关键指标上的差异（至少 3 项指标）
...
> **目标**: 在 `Architecture` 章节呈现具身智能主流技术路线与可落地工程架构，覆盖模型、系统与 AI Infra。

### 6.2.1 必须覆盖的技术路线（至少包含）
- **VLA / VLM-Robot 主干**：视觉-语言到动作的统一建模与控制接口
- **规划与控制协同**：高层 Planner + 低层 Controller 的分层架构
- **状态与记忆机制**：显式/隐式状态、可读写记忆、失败恢复与重规划
- **推理与系统工程**：端到端延迟预算、推理加速、实时控制稳定性

### 6.2.2 工程架构与落地工程（必须呈现）
- **端到端系统框图**：感知输入 → 语义/任务理解 → 规划 → 控制 → 执行反馈
- **关键工程指标**：任务成功率、恢复率、延迟、吞吐、安全事件率
- **AI Infra**：训练与推理基础设施（数据管线、作业编排、监控、回归评测）
...
> **目标**: 介绍具身智能所需的数据 Infra：真实采集、合成扩增、标注治理与质量控制。

### 6.3.1 必须覆盖的数据 Infra 主题（至少包含）
- **数据采集**：遥操作、在岗日志回流、多模态时间同步与坐标系对齐
- **数据合成**：可控环境合成、域随机化、长尾失败场景构造、反事实样本扩增
- **动作与状态标注**：动作标签、接触/力矩状态、任务阶段标签；必要时引入 VLM 辅助标注
- **数据存储与训练输入格式**：分片、索引、去重、版本化、可追溯元数据（配置/commit/hash）
- **质量控制与评测**：一致性检查、覆盖率、分布漂移监控、数据回归（数据变更引发指标波动）

### 6.3.2 输出形态要求
- 尽量用“工程架构图 + 流程图 + 对比表”组织（图文并茂优先）
- 给出可复用“方案模板”：采集 → 清洗 → 标注 → 版本化 → 训练输入 → 回归评测
...
□ 没有任何无效/空链接
□ 最后更新时间已刷新
□ 更新内容已写入主站更新日志（09_update_log.html）
□ 本次更新未触达 `legacy/`（强制）
```
...
### Changelog
- **v2.8 (2026-02-14)**:
  - 新增: 规则明确“后续常规更新强制忽略 `legacy/`（不扫描、不改写、不批量替换）”
  - 变更: 主站章节新增 `11_edge_chip.html` 并纳入章节结构与更新顺序
  - 变更: 强化“HTML 正文输出结果导向”，去除任务执行口吻与流程性表达
- **v2.7 (2026-02-14)**:
  - 新增: 主站章节 `11_edge_chip.html`（03.5 端侧芯片调研）并纳入强制更新顺序
  - 强化: 技术架构与数据工程章节深度要求（时延预算、部署门禁、仿真环境支持矩阵）
  - 强化: 论文追踪章节要求从“摘要式更新”提升为“可复现深度更新”
- **v2.6 (2026-02-14)**:
...
- **v2.5 (2026-02-14)**:
  ...
  - 优化: 产品、架构、数据工程规范改为具身智能目标（VLA、规划控制、采集/合成/标注闭环）
- **v2.4 (2026-02-14)**:
  - 重构: 规则由“双领域”切换为“具身智能单领域主站 + legacy 历史归档”
  - 变更: 更新顺序、更新日志规范、导航结构全部对齐主站章节
- **v2.3 (2026-02-14)**:
