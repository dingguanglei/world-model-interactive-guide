...
- 若必须修复 legacy 页面（例如失效链接/渲染崩溃），需在主站更新日志注明“归档修复”。
...
#### 6.3.1 Legacy 记录规范（强制）
- 与 legacy 目录相关的修复/迁移，也要在 `09_update_log.html` 中单独标注“Legacy”类别或说明。
...
□ 本次更新未误改 `legacy/`（除非任务明确要求）
□ 若涉及 legacy 修复，已在更新日志中明确标注
```
