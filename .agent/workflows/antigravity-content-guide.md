...
3. **🧠 AntiGravity's Commentary**: 必须给出专家点评（观点/判断/风险/边界），并明确区分事实与推断
...
<div style="display:flex; flex-direction:column; gap: 1rem;">
  ...
  <div style="background:#fce7f3; ...">
    <h4>🧠 AntiGravity's Commentary</h4>
    <p><strong>观点</strong>：这项工作真正值钱的不是画质，而是把“可交互”落到可复现的缓存/窗口机制上。</p>
    <p><strong>风险</strong>：论文的加速依赖特定算子与硬件；迁移到消费 GPU 可能出现延迟尖峰。</p>
  </div>
</div>
```
...
- 必须提供 **二级导航栏**（按年月分组锚点）
- 年月分组必须可折叠（`<details>`），并默认只展开最近 **3 个月**（其余收起）

### 4.2 论文卡片必备信息
| 字段 | 格式 |
|------|------|
| **标题** | **必须**可点击跳转到原文（优先 `arXiv/PDF`；其次 `官方 Project Page`） |
| **日期** | 📅 YYYY-MM-DD |
| **来源** | arXiv ID / Conference / Preprint / Project Page / GitHub（并在正文中标注“信息来自论文哪一章节/图表/表格”） |
| **标签** | NEW / GitHub / Conference |
| **代码开源** | **必须标注**：是否有公开 GitHub（有则给可点击链接；无则标注“未开源/未知”） |
| **参数开源** | **必须标注**：是否公开模型权重/检查点（优先链接到 HuggingFace/ModelScope/官方 Release/Checkpoint 指南；无则标注“未公开/未知”） |
| **数据集开源** | **必须标注**：训练数据是否开源/可公开获取（有则给可点击链接；否则明确标注“未公开/不可获取/未知”） |
| **论文主图** | **优先展示**论文主图（通常 Figure 1）：图片存入仓库 `world_model_interactive_guide/assets/papers/` 并在卡片中用 `<img>` 引用；figcaption 必须写清 “Figure X + 来源定位” |
...
| **公式渲染** | 如果出现公式：必须确保前端可读（推荐 MathJax；否则改写为纯文本/Unicode） |
...
- **🔴 专业解读**：拆解论文要点，至少覆盖：
  ...
  - **Limitations**（边界与失败模式）
  - 必要时举例，并配合图/表帮助理解。
- **🧠 AntiGravity's Commentary**：只写观点与判断，清晰标注假设与证据来源，不得混入未经来源支持的“事实”。

### 4.2.3 🔴 专业解析深度要求（强制，必须“可复现”）
...
- **最好增加**“问题 → 原因/瓶颈 → 方法机制 → 结果/代价”的因果逻辑图，帮助读者快速建立因果链路。

**引用定位（强制）**：
- 任何关键结论/数字/机制描述，必须标注来源位置，至少包含一种：
  - `Abstract`
  - `§x Section Title`（如 `§3 Method`）
  - `Fig.x / Table.x / Eq.x`
  - `Appendix`（如适用）
- 建议做成“来源定位”列表统一呈现（避免每句话都塞脚注导致可读性崩坏）。
...
### 4.4 Benchmarks（评测基准与评测方法）追踪规范（强制）
> **目标**：让读者看完就知道“这个 benchmark/方法在测什么、怎么测、测出来代表什么”，并能复现评测或至少复用评测脚本。

**4.4.1 章节位置与风格（强制）**
- Benchmarks 作为独立章节页面 `10_benchmarks.html`（章节编号为 **08**，与 `07_paper_tracker.html` 并列的一级章节），用于沉淀“通用评测基准/指标/方法”。
- 每条 benchmark/方法必须沿用论文追踪卡片风格：**🟢通俗 → 🔴专业 → 🧠 AntiGravity's Commentary（上下排列）**。

**4.4.2 每条条目必备信息（强制）**
- **标题**：必须可点击跳转到权威入口（优先：官方项目页/论文；其次：官方仓库）。
- **类型**：明确标注 `Benchmark / Dataset / Metric / Protocol`（可用 tag 展示）。
- **判定者（强制）**：必须用 tag 标注“最终分数/结论由谁判定”，三选一：
  - `判定者：模型`：用 VLM/LLM/Reward Model/Detector 等模型来打分（与“评测器模型”的理解能力强相关）
  - `判定者：人`：以人工主观评测/打分为准（需说明口径与人数/一致性）
  - `判定者：传统算法`：用固定公式/统计量/分布距离等计算（例如 FVD / 特征分布距离；与“评测器模型”无关）
- **来源与链接**（尽可能齐全）：
  - 论文（arXiv/会议/OpenReview）链接
  - 官方项目页链接（如有）
  - 仓库链接（GitHub 等，如有）
  - 数据集链接（HF Datasets/官方下载页等；若未公开必须明确写“未公开/不可获取/未知”）
- **评测重点（What it measures）**：至少 3 条要点，说明该基准主要诊断什么能力/失败模式（例如主体一致性、长时漂移、物理合理性、任务成功率）。
- **使用方式（How to use）**：给出可执行的最小复现路径（至少包含“输入是什么、输出是什么、关键配置是什么”，并尽量链接到仓库 README/脚本入口）。
- **局限与误用风险（Pitfalls）**：至少 1 条（例如自动指标偏差、可被刷分、对特定失败模式不敏感、评测口径不可比等）。
- **来源定位（强制）**：像论文追踪一样标注来源位置（Abstract/§x/Fig.x/README 某段/脚本路径等），避免只贴一个总链接。

**4.4.3 🧠 Commentary 写法（强制）**
- 必须给出“该评测对世界模型/交互视频的真实价值”与“最容易踩的坑/刷分风险”，并建议如何把它用作工程回归测试（regression）而不是营销 KPI。
...
- **引用链接（尽可能）**: 原帖/原文/被讨论论文或项目，尽可能提供可点击超链接（tweet/Reddit thread/HF discussion/博客文章等）
- **三段解读（强制）**: 🟢通俗 → 🔴专业 → 🧠 AntiGravity's Commentary（上下排列）
...
```
- **日期刷新口径（重要，避免误导）**：
  - 只有当**该章节页面的内容发生实质更新**（新增/修正事实、补充来源、修复图表/代码块导致的渲染问题等）时，才允许刷新该页的 `最后更新时间` 日期
  - **纯样式/排版统一**（例如仅把样式迁移到 `css/style.css`、统一 UI 组件外观、调整间距/颜色但不改变信息内容）**不得**刷新该页 `最后更新时间`
  - 若当日仅做“样式统一/工程维护”，应仅在 `09_update_log.html` 记录，并由更新日志页刷新其 `最后更新时间`
- **样式统一（重要）**：
...
□ `最后更新时间` 只在该章节内容实质更新时刷新；纯样式/排版统一不刷新（只更新更新日志页）
